...
function markdown_2_toc(markdown) {
    ...
    toc += "Go back to Table of Contents";
    toc += "</span></div>";
    ...
    for (let line_i = 0; line_i < (lines.length - 1); line_i++) {
        ...
        if (hdr_chk == "EX:") {
            display_All = line.substring(3).trim();
        };
...
function markdown_2_page(markdown) {
    ...
    for (let line_i = 0; line_i < (lines.length - 1); line_i++) {
        ...
        if (hdr_chk == "h1:") {
            h1_count += 1;
            hdr_name = line.substring(3);
            if (h1_count == display_chapter || display_All == "A") {
                page_build += '<h1>' + line.substring(3) + '</h1>';
            };
            continue;
        };
        if (h1_count == display_chapter || display_All == "A") {
    ...
    page_build += '<span onclick="window.scrollTo(0, 0)" style="cursor: pointer; color: blue; text-decoration: underline; font-size: 18px;">[Back to Top]</span>';
...
function GoToChapter(inVal) {
    ...
    convertMarkdown()
    const verseElement = document.getElementById("html-page");
